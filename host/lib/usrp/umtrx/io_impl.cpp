...
static const uhd::dict<std::string, int> subdev_names_map = map_list_of
    ("0", 1)
    ("1", 2)
    ("2", 3)
    ("3", 4)
...
//    boost::uint32_t dsp_mapping = _mbc[which_mb].iface->peek32(U2_REG_SR_ADDR(SR_RX_FRONT_SW));
    ...
    for (size_t i = 0; i < spec.size(); i++){
        ...
        boost::uint32_t val = (spec[0].db_name == "A")?0:1;
        dsp_mapping |= (val << subdev_names_map[spec[0].sd_name]);
    }
